...
# tiers_file = '~/my_tmpstier_splits_newT.tsv'
$ python ticket_forecast.py run_date rerun
...
def get_last_sat(r_date):
    # get last Saturday date strictly before r_date.
    # If r_date is a Sunday or Monday give the date of 2 Saturday prior to r_date
    # Otherwise the Sat prior to r_date
    dt = pd.to_datetime(r_date)
    inc = 7 if dt.weekday() in [0] else 0   # 7 for Mon, Tues
    sat = dt - pd.to_timedelta(inc + dt.weekday() + 2, unit='D')
    return sat
...
def data_clean(dt, pre_sat):
    ...
    dt['ds_week_ending'] = pd.to_datetime(dt['ds_week_ending'])
    dt = dt[dt['ds_week_ending'] <= pre_sat].copy()
    if 'ds_week_starting' not in dt.columns:
        dt['ds_week_starting'] = dt['ds_week_ending'] - pd.to_timedelta(6, unit='D')
...
def tier_fill(df):
    ...
    df['dim_tier'] = df['dim_tier'].apply(lambda x: 'plus' if x in ['a4w'] else x)
    df['dim_tier'] = df['dim_tier'].apply(lambda x: 'safety' if x in ['law-enforcement'] else x)
    df['dim_tier'] = df['dim_tier'].apply(lambda x: 'payments' if x in ['payments-and-payouts'] else x)
    ...
    df_adj = df[df['dim_tier'].apply(lambda x: x not in ['experiences', 'plus'])]
    df_nounk = df_adj[df_adj['dim_tier'] != '-unknown-'].copy()

    dfg_adj = df_adj.groupby(['ds_week_starting', 'dim_business_unit', 'dim_language']).sum().reset_index()
    dfg_nounk = df_nounk.groupby(['ds_week_starting', 'dim_business_unit', 'dim_language']).sum().reset_index()
    merged = pd.merge(dfg_adj, dfg_nounk, on=['ds_week_starting', 'dim_business_unit', 'dim_language'], suffixes=('_all', '_adj'))
    merged['adj'] = (merged['ticket_count_all'] + 0.01) / (merged['ticket_count_adj'] + 0.01)
    del merged['ticket_count_all'], merged['ticket_count_adj']
    df_nounk = pd.merge(df_nounk, merged, on=['ds_week_starting', 'dim_business_unit', 'dim_language'])
...
def group_agg(df, level):
    ...
    print(df.head())
    if level == 'bu':
        l_grouped = df.groupby(['ds_week_starting', 'dim_business_unit']).sum().reset_index()
        l_grouped['pert'] = l_grouped[['ds_week_starting', 'ticket_count']].groupby(['ds_week_starting']).transform(lambda x: x.sum())
    elif level == 'bu_lang':
        l_grouped = df.groupby(['ds_week_starting', 'dim_business_unit', 'dim_language']).sum().reset_index()
        l_grouped['pert'] = l_grouped[['ds_week_starting', 'dim_business_unit', 'ticket_count']].groupby(['ds_week_starting', 'dim_business_unit']).transform(lambda x: x.sum())
    elif level == 'bu_lang_tier':
        l_grouped = df.groupby(['ds_week_starting', 'dim_business_unit', 'dim_language', 'dim_tier']).sum().reset_index()
        ...
        l_grouped = df.groupby(['ds_week_starting', 'dim_business_unit', 'dim_language', 'dim_channel']).sum().reset_index()
        l_grouped['pert'] = l_grouped[['ds_week_starting', 'dim_business_unit', 'dim_language', 'ticket_count']].groupby(
            ['ds_week_starting', 'dim_business_unit', 'dim_language']).transform(lambda x: x.sum())
    elif level == 'bu_lang_tier_channel':
        l_grouped = df.groupby(['ds_week_starting', 'dim_business_unit', 'dim_language', 'dim_tier', 'dim_channel']).sum().reset_index()
...
def arima_forecast_noex(data, fcst_end, wid, sea='y', trans='y', tr_lda=None):
    ...
    output = pd.concat(output_list, axis=1)
    output.columns = col_list
    arr = [pd.to_datetime(x) for x in output.index]
    output.index = arr
    return output
...
def arima_forecast(data, fcst_end, sea='y', trans='y', exog=None, wid=5, tr_lda=None):
    ...
    if exog is None:
        ...
        foutput[foutput < 0.0] = 0
        p_ut.save_df(foutput, '~/my_tmp/foutput')
        foutput.index = np.array([x if isinstance(x, str) else str(x.date()) for x in foutput.index])
        return foutput
    ...
    foutput[foutput < 0.0] = 0
    foutput.index = [pd.to_datetime(x) for x in foutput.index]
    return foutput
...
def forecast_bu_lang_arima(bl_grouped, fcst_end_date, cb, noex_lang_lst, run_date):
    ...
    for (bu, lang) in bl_grouped.groupby(['dim_business_unit', 'dim_language']).groups.keys():
        ...
        tr_lda = None
        wid = 5
        input_data = bl_grouped[(bl_grouped['dim_business_unit'] == bu) & (bl_grouped['dim_language'] == lang)][['ticket_count']]
        print('processing at language level : ' + bu + ' - ' + lang)
        # in case that certain language does have enough long history as checkins
        if len(cb) > len(input_data):
            idx = input_data.index
...
#                input_data['ticket_count'], fcst_end_date, wid=wid, exog=exog, tr_lda=tr_lda)
#        else:
        f = arima_forecast(input_data['ticket_count'], fcst_end_date, wid=wid, exog=exog, tr_lda=tr_lda)
        p_ut.save_df(f, '~/my_tmp/tix_f')
...
def pct_to_excel(bltc_grouped, pct_bu_lang_tier_channel, hist_cut_off_date):
    ...
    output_pct = pd.concat([hist_pct, fcst_pct], sort=False)
...
def rs_to_excel(hist_ticket, fcst_ticket, hist_cut_off_date, r_date):
    ...
    hist_ticket['ds_week_ending'] = hist_ticket['ds_week_starting'].apply(lambda x: datetime.strftime(
        (datetime.strptime(str(x.date()), '%Y-%m-%d') + timedelta(days=6)), '%Y-%m-%d'))
...
def adjust_ag_tiers(zx, r_date, weeks):
    # adjust dim_tier counts for the last 2 weeks, ie. when ds_week_ending equals run_date and run_date - 1 week
    ...
    return pd.concat([w_old, f[cols].copy()], axis=0), gm


def run_hql_(q_files, d_file):
    try:
        return hql.run_hql(q_files, d_file)
    except RuntimeError as e:
        s_ut.my_print('pid: ' + str(os.getpid()) + ' run_hql runtime error: ' + str(e))
        s_ut.my_print('pid: ' + str(os.getpid()) + ' checking if  ' + d_file + ' exists')
        ret, _ = hql.find_outdata_file(d_file)
        return ret


def get_ratios(df_, cutoff_date):
    df = df_.copy()
    if 'ds_week_starting' not in df.columns:
        df.reset_index(inplace=True)
    df['ds_week_starting'] = pd.to_datetime(df['ds_week_starting'])
    df = df[df['ds_week_starting'] >= cutoff_date - pd.to_timedelta(6, unit='W')].copy()
    bl_df = df.groupby(['dim_language', 'dim_business_unit']).agg({'ticket_count': np.sum}).reset_index()
    blt_df = df.groupby(['dim_language', 'dim_business_unit', 'dim_tier']).agg({'ticket_count': np.sum}).reset_index()
    dft = bl_df.merge(blt_df, on=['dim_language', 'dim_business_unit'], how='left')
    dft['ratio'] = dft['ticket_count_y'] / dft['ticket_count_x']
    dft.drop(['ticket_count_x', 'ticket_count_y'], axis=1, inplace=True)
    bltc_df = df.groupby(['dim_language', 'dim_business_unit', 'dim_tier', 'dim_channel']).agg({'ticket_count': np.sum}).reset_index()
    dfc = blt_df.merge(bltc_df, on=['dim_language', 'dim_business_unit', 'dim_tier'], how='left')
    dfc['ratio'] = dfc['ticket_count_y'] / dfc['ticket_count_x']
    dfc.drop(['ticket_count_x', 'ticket_count_y'], axis=1, inplace=True)
    return dft, dfc


def dict_2_df(df_dict):
    df_list = list()
    for k, v in df_dict.items():
        f = pd.DataFrame(v)
        f.reset_index(inplace=True)
        f.columns = ['ds_week_starting', 'ticket_count']
        f['dim_business_unit'] = k[0]
        f['dim_language'] = k[1]
        df_list.append(f)
    return pd.concat(df_list, axis=0)


def set_cols(df_, ratio_df, gcols):
    z = ratio_df.merge(df_, on=gcols, how='left')
    z['ticket_count'] *= z['ratio']
    z['ticket_count'].fillna(0, inplace=True)
    z.drop('ratio', axis=1, inplace=True)
    return z
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 2:
        ...
        _, run_date, rerun = sys.argv  # at least 3 days after last Saturday with actual data
    else:
    ...
    s_ut.my_print('----- start forecast pipeline from cutoff date ' + str(run_date.date()) + ' to forecast date ' + str(fcst_end_date.date()) + '  -----')

    # output files
    out_file = os.path.expanduser('~/Forecasts/par/' + 'table_output_' + str(run_date.date()))
    ...
    q_dir = '~/my_repos/capacity_planning/'
    tix_qry_file = os.path.expanduser(q_dir + 'data/vol/ticket_forecast/tickets.hql')
    tiers_qry_file = os.path.expanduser(q_dir + 'data/vol/ticket_forecast/tier_splits.hql')
    b_qry_file = os.path.expanduser(q_dir + 'data/vol/ticket_forecast/bookings_and_checkins.hql')
    zd_tiers_qry_file = os.path.expanduser(q_dir + 'data/vol/ticket_forecast/zendesk-tiers.hql')

    tix_qfiles = hql.gen_query(tix_qry_file, 'tickets_qry', set_dict, date=str(run_date.date()), gb_replace=False)
    tiers_qfiles = hql.gen_query(tiers_qry_file, 'tiers_qry', set_dict, date=str(run_date.date()), gb_replace=False)
    b_qfiles = hql.gen_query(b_qry_file, 'bk_qry', set_dict, date=str(run_date.date()), gb_replace=False)
    z_qfiles = hql.gen_query(zd_tiers_qry_file, 'zd_qry', set_dict, date=str(run_date.date()), gb_replace=False)
    ...
    f_list = s_ut.do_mp(run_hql_, arg_list, is_mp=True, cpus=None, do_sigkill=True, start_method='spawn')
    ...
    for f in f_list:
        if f == -1:
            ...
            f = '~/my_tmp/tier_splits_newT_2019-10-12.par'
            # sys.exit()
        if 'ticket_by_bus_channel_language_nt' in f:
            ...
            zd_df, _ = adjust_ag_tiers(zd_df_, run_date, 16)           # weeks = max weeks back from run_date
    ...
    dft, dfc = get_ratios(df, run_date)
    p_ut.save_df(dft, '~/my_tmp/dft_' + str(run_date.date()))
    p_ut.save_df(dfc, '~/my_tmp/dfc_' + str(run_date.date()))

    if 'ds_week_starting' not in df.columns:
        df.reset_index(inplace=True)
    p_ut.save_df(df, '~/my_tmp/cleaned/old_tickets_' + str(run_date.date()))
    ...
    # get the ratios from actuals for the last 4 weeks
    ...
    print('==== start language + bu level forecast === with fcat end date ' + str(fcst_end_date.date()))
    ...
    print('=========== set tier and channel by ratios =========')
    bl_fcast_df = dict_2_df(rs_bu_lang)
    p_ut.save_df(bl_fcast_df, '~/my_tmp/bl_fcast_df')
    bl_fcast_df = p_ut.read_df('~/my_tmp/bl_fcast_df')
    bl_fcast_df['ds_week_starting'] = pd.to_datetime(bl_fcast_df['ds_week_starting'])

    # bl_fcast_df['ds_week_starting'] = pd.Timestamp(bl_fcast_df['ds_week_starting'])
    print(bl_fcast_df.dtypes)
    dtypes = [type(x) for x in list(bl_fcast_df['ds_week_starting'])]
    utypes = set(dtypes)
    print(utypes)
    for ut in utypes:
        cnt = 0
        for x in list(bl_fcast_df['ds_week_starting']):
            if isinstance(x, ut):
                cnt += 1
        print(str(ut) + ': ' + str(cnt))
    # bl_fcast_df = bl_fcast_df[bl_fcast_df['ds_week_starting'] > str(run_date.date())].copy()  # bug somewhere and actual are here also
    bl_fcast_df = bl_fcast_df[bl_fcast_df['ds_week_starting'] > run_date.date()].copy()  # bug somewhere and actual are here also
    p_ut.save_df(bl_fcast_df, '~/my_tmp/bl_fcast_df_' + str(run_date.date()))
    blt_fcast_df = set_cols(bl_fcast_df, dft, ['dim_business_unit', 'dim_language'])
    p_ut.save_df(blt_fcast_df, '~/my_tmp/blt_fcast_df_' + str(run_date.date()))
    bltc_fcast_df = set_cols(blt_fcast_df, dfc, ['dim_business_unit', 'dim_language', 'dim_tier'])
    p_ut.save_df(bltc_fcast_df, '~/my_tmp/bltc_fcast_df_' + str(run_date.date()))

    # add cols to go to table
    bltc_fcast_df['time_interval'] = 'week'
    bltc_fcast_df['run_date_inv_ending'] = run_date
    bltc_fcast_df['ds_week_starting'] = pd.to_datetime(bltc_fcast_df['ds_week_starting'])
    bltc_fcast_df['fcst_date_inv_ending'] = bltc_fcast_df['ds_week_starting'] + pd.to_timedelta(6, unit='D')
    bltc_fcast_df['fcst_horizon'] = ((bltc_fcast_df['fcst_date_inv_ending'] - bltc_fcast_df['run_date_inv_ending']).dt.days / 7.0).astype(int)

    # bltc_fcast_df['fcst_date_inv_ending'] = idx_to_week_ending(bltc_fcast_df.index)
    # bltc_fcast_df['fcst_horizon'] = pd.to_datetime(bltc_fcast_df['fcst_date_inv_ending']) - pd.to_datetime(bltc_fcast_df['run_date_inv_ending'])
    # bltc_fcast_df['fcst_horizon'] = bltc_fcast_df['fcst_horizon'].apply(lambda x: x.total_seconds() / (3600 * 24 * 7) - 1)
    print(bltc_fcast_df.head())
    cols = ['dim_business_unit', 'dim_language', 'dim_tier', 'dim_channel',
            'time_interval', 'run_date_inv_ending', 'fcst_date_inv_ending', 'fcst_horizon', 'ticket_count']
    ...
    table_output = bltc_fcast_df[cols].copy()
    table_output['fcst_date_inv_ending'] = table_output['fcst_date_inv_ending'].dt.date.astype(str)
    table_output['run_date_inv_ending'] = table_output['run_date_inv_ending'].dt.date.astype(str)
    p_ut.save_df(table_output, out_file)

    # ############################################

    # final file
    # p_ut.save_df(df, '~/my_tmp/final_df_' + str(run_date.date()))   # actuals from beginning of time correct at language level wit ds_week_starting
    start_date = str(zd_df.ds_week_starting.min())
    # xls_bu_lang_tier_channel = rs_to_excel(zd_df, table_output, start_date, run_date)  # KEEP: merges historical and fcast!!!!!!
    xls_bu_lang_tier_channel = rs_to_excel(actuals_df, table_output, start_date, run_date)  # KEEP: merges historical and fcast!!!!!!
    ...
    xlwriter.save()
    s_ut.my_print('excel file: ' + xls_file)
    # ret = t2t.main(str(run_date.date()))   # upload to ticket forecast table
    print('DONE')
    # if ret == 0:
    #     print('DONE')
    # else:
